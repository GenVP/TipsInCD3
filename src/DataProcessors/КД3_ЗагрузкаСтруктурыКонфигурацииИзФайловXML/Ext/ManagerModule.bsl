...
Процедура ОбработатьКаталогиСДанными(ОбщиеПеременные)
	...
	Если КД3_Настройки.ИспользоватьКонтекстнуюПодсказку() И НЕ ОбщиеПеременные.ТолькоОбновитьПланыОбмена Тогда
		КД3_ЗагрузкаМетаданных.ОбработатьГруппуОбщихМодулей(ОбщиеПеременные, ОбщиеПеременные.КД3_ЗагружатьМетодыМодулей);
	КонецЕсли;
	//-КД3
КонецПроцедуры
		...
		Иначе
			...
			Если ОбщиеПеременные.КД3_ЗагружатьМетодыМодулей Тогда
				...
				Для Каждого ВидМодуля Из ВидыМодулей Цикл
					...
					ПолноеИмяМетаданных = НРег(СтрШаблон("module.%1.%2.%3", ВидМодуля, НРЕг(ИмяГруппы), ИмяОбъекта));
					ОписаниеМетаданных = КД3_ЗагрузкаМетаданных.ЗагрузитьМодульОбъекта(ОбщиеПеременные, ОбщиеПеременные.Конфигурация, ВидМодуля, НРЕг(ИмяГруппы), ИмяОбъекта);
					Если ОписаниеМетаданных <> Неопределено И ОписаниеМетаданных.count > 0 Тогда
						КД3_Метаданные.ИзменитьОписаниеМетаданных(ОбщиеПеременные.Конфигурация, ПолноеИмяМетаданных, ОписаниеМетаданных, ОбщиеПеременные.КД3_НастройкиКонфигурации);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			//-КД3
