...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаНастройкиРедактора.Доступность = ИспользоватьРедакторКода;
...
&НаКлиенте
Асинх Процедура СохранитьКонсольКодаWin(Команда)
	
	ДанныеКонсоли = ДанныеКонсолиКодаНаСервере(Ложь, УникальныйИдентификатор);
	
	ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
	ДиалогВыбора.Заголовок = "Сохранение архива исходников bsl_console";
	ДиалогВыбора.Фильтр = "Архив (*.zip)|*.zip|Все файлы (*.*)|*.*";
	ДиалогВыбора.ПолноеИмяФайла = ДанныеКонсоли.Версия;
	
	Результат = Ждать ДиалогВыбора.ВыбратьАсинх();
	Если Результат <> Неопределено Тогда
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(ДанныеКонсоли.Адрес);
		Ждать ДвоичныеДанные.ЗаписатьАсинх(Результат[0]);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДанныеКонсолиКодаНаСервере(ЭтоЛинуксКлиент, УникальныйИдентификаторФормы)
	
	ДанныеКонсоли = Новый Структура;
	ДанныеКонсоли.Вставить("Адрес", КД3_Настройки.ДвоичныеДанныеКонсолиКода(ЭтоЛинуксКлиент));
	ДанныеКонсоли.Вставить("Версия", КД3_Настройки.ВерсияКонсолиКода(ЭтоЛинуксКлиент).Версия);
	
	Возврат ДанныеКонсоли;
