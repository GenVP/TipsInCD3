...
Функция ОписаниеГлобальногоКонтекстаПоЗапросу(Конфигурация, НастройкиКонфигурации) Экспорт
	...
	Иначе
		...
		ОписаниеОбщихМодулей.Вставить("ОбщиеМодули", ОписаниеМетаданных(Конфигурация, "commonmodules.items"));
		ОписаниеОбщихМодулей.Вставить("ГлобальныеМодули", ОписаниеМетаданных(Конфигурация, "globalfunctions"));
	КонецЕсли;
...
Функция ОписаниеМетаданныхПоЗапросу(Конфигурация, ПолноеИмяМетаданных, НастройкиКонфигурации) Экспорт
	...
	Если ОписаниеМетаданных = Неопределено Тогда
		ОписаниеМетаданных = Новый Структура("count", 0); // Пустое описание метаданных для сохранения в кэше
	КонецЕсли;
...
Функция ОписаниеМетаданных(Конфигурация, КлючКоллекции, НастройкиКонфигурации = Неопределено) Экспорт
...
Функция НастройкиКонфигурации(Конфигурация) Экспорт
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Настройки = Выборка.Данные.Получить();
	Иначе
...
Функция ЗагрузитьГлобальныйКонтекстИзФайлов(Конфигурация, ЗагружатьМетодыМодулей)
	...
	РеквизитыКонфигурации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Конфигурация, "ИсточникДанных,КаталогЗагрузки");
	
	ПараметрыЗагрузки = Новый Структура;
	ПараметрыЗагрузки.Вставить("ИсточникДанных", РеквизитыКонфигурации.ИсточникДанных);
	ПараметрыЗагрузки.Вставить("КаталогЗагрузки", РеквизитыКонфигурации.КаталогЗагрузки);
	...
	ОписаниеОбщихМодулей = Новый Структура;
	ОписаниеОбщихМодулей.Вставить("ГлобальныеМодули", ОбъектВСтрокуJSON(КД3_ЗагрузкаМетаданных.КоллекцияОбъектов(ПараметрыЗагрузки, "globalfunctions")));
	ОписаниеОбщихМодулей.Вставить("ОбщиеМодули", ОбъектВСтрокуJSON(КД3_ЗагрузкаМетаданных.КоллекцияОбъектов(ПараметрыЗагрузки, "commonModules.items")));
	
	Возврат ОписаниеОбщихМодулей;
	
КонецФункции

Функция ЗагрузитьКоллекциюОбъектовИзФайлов(Конфигурация, КлючКоллекции)
	...
	ОписаниеМетаданных = КД3_ЗагрузкаМетаданных.ОбработатьГруппуОбъектов(ПараметрыЗагрузки, КлючКоллекции);
	
	Возврат ОписаниеМетаданных;
	
КонецФункции

Функция ЗагрузитьОбъектИзФайла(Конфигурация, КлючКоллекции, ИмяОбъекта)
	...
	ОписаниеМетаданных = КД3_ЗагрузкаМетаданных.ОбработатьОбъект(ПараметрыЗагрузки, КлючКоллекции, ИмяОбъекта);
...
Функция ЗагрузитьМодульОбъектаИзФайла(Конфигурация, ВидМодуля, КорневойТип, ИмяОбъекта)
	...
	Возврат КД3_ЗагрузкаМетаданных.ЗагрузитьМодульОбъекта(ПараметрыЗагрузки, Конфигурация, ВидМодуля, КорневойТип, ИмяОбъекта);
