...
Процедура ИзвлечьИсходники() Экспорт
	...
	Если КаталогИсходников = Неопределено Тогда
		...
		КаталогИсходников = КаталогВременныхФайлов.Путь + ПодкаталогИсходников + РазделительПути;
		...
		Для Каждого Файл Из СтарыеВерсии Цикл
			УдалитьФайлы(Файл.ПолноеИмя + РазделительПути, "*.*");
		КонецЦикла;
		СоздатьКаталог(КаталогИсходников);
		
		// Получение файла макета общего для всех конфигураций
		КаталогОбработчика = КаталогИсходников + "CommonEvent" + РазделительПути;
