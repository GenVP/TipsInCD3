...
Процедура ОбработатьГруппуОбщихМодулей(ОбщиеПеременные, ЗагружатьМетодыНеГлобальныхМодулей = Ложь) Экспорт
	...
	Иначе
		ВложенныеФайлы = НайтиПомещенныеФайлыПоМаске(ОбщиеПеременные.ПомещенныеФайлы, Разделитель + "CommonModules" + Разделитель, ".mdo", ОбщиеПеременные);
	КонецЕсли;
...
Функция ЗагрузитьМодульОбъекта(ОбщиеПеременные, Конфигурация, ВидМодуля, КорневойТип, ИмяОбъекта) Экспорт
	...
	Иначе
		ФайлОбъекта = НайтиПомещенныеФайлыПоМаске(ОбщиеПеременные.ПомещенныеФайлы, Разделитель + СтрокаТипа.МнEng + Разделитель, ИмяОбъекта + ".mdo", ОбщиеПеременные);
	КонецЕсли;
	Если ФайлОбъекта.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	ТекстФайл = Новый Файл(ФайлОбъекта[0].ТекущееИмяФайла);
...
Процедура ЗаполнитьПараметрыМетодаПоОписанию(КД3_Вычислитель, СтруктураПараметров, ОписаниеПараметры)
	...
	Пока ИндексСтроки < КоличествоСтрок Цикл
		...
		КонецЕсли;
		//+ТЕСТ
		ОписаниеПараметра = СтрЗаменить(ОписаниеПараметра, СтрокаТЗ.Имя + " - ", "");
		//-ТЕСТ
...
Функция ПрочитатьАтрибуты(ЧтениеXML, СтруктураРеквизитов, ИмяГруппы)
...
Функция ПрочитатьСвойствоСтруктуры(ЧтениеXML, ОписаниеЭлемента, ОсталосьСвойств = 0)
