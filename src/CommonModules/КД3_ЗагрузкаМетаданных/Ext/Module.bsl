	...
	Иначе
		...
		Для Каждого КлючИЗначение Из ОбщиеПеременные.КД3_ПрогрессСоставРасшифровки Цикл
			Если НЕ ПустаяСтрока(КлючИЗначение.Значение) Тогда
				СоставРасшифровки.Добавить(КлючИЗначение.Значение);
			КонецЕсли;
		КонецЦикла;
		Если СоставРасшифровки.Количество() = 1 И СтрЗаканчиваетсяНа(СоставРасшифровки[0], ": ") Тогда
			// Вывод процента расшифровки вместе с сообщением основного прогресса (ПрогрессКлюч = "" для НачатьПрогрессРасшифровки)
		...
		КонецЕсли;
	КонецЕсли;
	ДатаНовогОСообщения = ТекущаяДатаСеанса();
	Если ОбщиеПеременные.КД3_ДатаПоследнегоСообщения = Неопределено ИЛИ ОбщиеПеременные.КД3_ДатаПоследнегоСообщения + 4 < ДатаНовогОСообщения Тогда
		ОбщиеПеременные.КД3_ДатаПоследнегоСообщения = ДатаНовогОСообщения ;
		ДлительныеОперации.СообщитьПрогресс(ОбщиеПеременные.КД3_ПрогрессПроцент, ПрогрессТекст);
	КонецЕсли;
	
КонецПроцедуры

Процедура НачатьПрогрессРасшифровки(ОбщиеПеременные, КоличествоОбъектов, ПрогрессКлюч = "", ПрогрессТекст = "") Экспорт
	...
	ДанныеПрогресса.Вставить("КД3_ПрогрессКлюч", ПрогрессКлюч);
	Если ПустаяСтрока(ПрогрессТекст) Тогда
		ДанныеПрогресса.Вставить("КД3_ПрогрессТекст", ПрогрессКлюч);
	Иначе
...
Процедура НачатьПолучениеРасшифровки(ОбщиеПеременные, ПрогрессКлюч) Экспорт
	
	ОбщиеПеременные.КД3_ПрогрессСоставРасшифровки.Вставить(ПрогрессКлюч, "");
...
Процедура УвеличитьСообщитьПрогрессРасшифровки(ОбщиеПеременные) Экспорт
	...
	ПрогрессПроцентИТекст = СтрШаблон("%1%%: %2", Формат(ДанныеПрогресса.КД3_ПрогрессПроцент, "ЧЦ=2; ЧН=00; ЧВН="), ДанныеПрогресса.КД3_ПрогрессТекст);
	Если НЕ ОбщиеПеременные.КД3_ОсновнойПоток Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон("{КД3}#%1#%2", ДанныеПрогресса.КД3_ПрогрессКлюч, ПрогрессПроцентИТекст);
		Сообщение.Сообщить();
	Иначе
		ОбщиеПеременные.КД3_ПрогрессСоставРасшифровки.Вставить(ДанныеПрогресса.КД3_ПрогрессКлюч, ПрогрессПроцентИТекст);
		СообщитьПрогрессОбщий(ОбщиеПеременные);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗавершитьПолучениеРасшифровки(ОбщиеПеременные, ПрогрессКлюч) Экспорт
	
	Если НЕ КД3_ЗагрузкаМетаданных.НужноСообщатьПрогресс(ОбщиеПеременные) Тогда
		Возврат;
	КонецЕсли;
	ОбщиеПеременные.КД3_ПрогрессСоставРасшифровки.Удалить(ПрогрессКлюч);
...
Процедура ПрочитатьПрогрессРасшифровки(ОбщиеПеременные, Сообщение) Экспорт
	...
	ПрогрессКлюч = ОписаниеПрогресса[1];
	ПрогрессПроцентИТекст = ОписаниеПрогресса[2];
	
	Если ОбщиеПеременные.КД3_ПрогрессСоставРасшифровки[ПрогрессКлюч] <> Неопределено Тогда
		ОбщиеПеременные.КД3_ПрогрессСоставРасшифровки.Вставить(ПрогрессКлюч, ПрогрессПроцентИТекст);
	КонецЕсли;
...
Функция НайтиПомещенныеФайлыПоМаске(ПомещенныеФайлы, ВложенныйКаталог, ИмяФайла, ПараметрыЗагрузки) Экспорт
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("ИсходноеИмяФайла");
	Результат.Колонки.Добавить("ТекущееИмяФайла");
	
	ИсключитьВложенныеКаталоги = (СтрНайти(ИмяФайла, ".mdo") = 0);
	...
	Если ПомещенныеФайлы = Неопределено Тогда
		...
		Если ПараметрыЗагрузки.ЭтоРасширение Тогда
			НайденныеФайлы = НайтиФайлы(ПараметрыЗагрузки.ПутьКФайламРасширения + ВложенныйКаталог, ИмяФайла, НЕ ИсключитьВложенныеКаталоги);
		Иначе
			НайденныеФайлы = НайтиФайлы(ПараметрыЗагрузки.КаталогЗагрузки + ВложенныйКаталог, ИмяФайла, НЕ ИсключитьВложенныеКаталоги);
		КонецЕсли;
	...
	Иначе
		...
		Для Каждого ТекФайл Из ПомещенныеФайлы Цикл
			...
			ПозицияРасширения = СтрНайти(ТекФайл.ПолноеИмя, ИмяФайла);
...
Процедура ОбработатьГруппуОбъектовМногопоточно(ОбщиеПеременные, ПараметрыЗадания, АдресРезультата) Экспорт
	
	Обработки.КД3_ЗагрузкаСтруктурыКонфигурацииИзФайловXMLМногопоточно.КД3_ОбработатьГруппуОбъектовМногопоточно(ОбщиеПеременные, ПараметрыЗадания, АдресРезультата);
...
Процедура НачатьПрогресс(ДанныеПрогресса, КоличествоОбъектов)
	...
	ДанныеПрогресса.Вставить("КД3_ДатаПоследнегоСообщения", Неопределено);
