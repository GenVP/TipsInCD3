...
Функция ДоступныеКонфигурации(Форма, ЭтоКонвертацияXDTO)
	...
	Если Форма.ИспользованиеДляГруппИЭлементов = ИспользованиеГруппИЭлементов.Группы Тогда
	...
	Иначе
		Если ПроверкаРеквизита.СписокКонвертаций = Неопределено Тогда
			...
			ПроверкаРеквизита.СписокКонвертаций = Новый СписокЗначений;
			КонвертацияДанныхXDTOВызовСервера.ПолучитьВхожденияВКонвертацииДляЭлементаКонвертации(
				Форма.Объект.Владелец, ПроверкаРеквизита.СписокКонвертаций);
		КонецЕсли;
		ДоступныеКонфигурации = КД3_Метаданные.ПолучитьКонфигурации(ПроверкаРеквизита.СписокКонвертаций, ЭтоКонвертацияXDTO);
		ДоступныеКонфигурации.Вставить("КонфигурацияЕстьНаФорме", Ложь);
	КонецЕсли;
